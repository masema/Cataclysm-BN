...
TEST_CASE("default season temperatures", "[weather]") {
    ...
    for (size_t current_season = 0; current_season < static_cast<size_t>(NUM_SEASONS);
        ...
        const time_point start_season_time =
            calendar::turn_zero + current_season * calendar::season_length();
        const time_point end_season_time =
            calendar::turn_zero + next_season * calendar::season_length();
        const units::temperature min_temperature =
            std::min(season_stats[current_season].average_temperature,
                     season_stats[next_season].average_temperature)
            - max_offset;
        const units::temperature max_temperature =
            std::max(season_stats[current_season].average_temperature,
                     season_stats[next_season].average_temperature)
            + max_offset;
        constexpr const tripoint_abs_ms pos;
        for (time_point current_time = start_season_time; current_time < end_season_time;
             current_time += time_duration::from_hours(1)) {
            CAPTURE(current_season);
            const double season_progress =
                (current_time - start_season_time) / calendar::season_length();
            ...
            CHECK(generator.get_weather_temperature(pos, current_time, calendar::config, seed)
                  >= min_temperature);
            CHECK(generator.get_weather_temperature(pos, current_time, calendar::config, seed)
                  <= max_temperature);
        }
...
TEST_CASE("water temperatures track season temperatures", "[weather]") {
    ...
    const auto spring_calendar =
        calendar_config(calendar::turn_zero, calendar::turn_zero, SPRING, true);
    const auto summer_calendar =
        calendar_config(calendar::turn_zero, calendar::turn_zero, SUMMER, true);
    const auto winter_calendar =
...
// https://gist.github.com/Kodiologist/e2f1e6685e8fd865650f97bb6a67ad07
{
    ...
    for (auto seed : seeds) {
        ...
        for (time_point i = begin; i < end; i += 1_minutes) {
            ...
            hourly_precip[hour] += precip_mm_per_hour(wgen.get_weather_conditions(w)->precip) / 60;
        }
        ...
        for (int do_highs = 0; do_highs < 2; ++do_highs) {
            ...
            std::transform(
                temperature.begin(), temperature.end(), t.begin(),
                [&](std::vector<double> const& day) {
                    return do_highs ? *std::max_element(day.begin(), day.end())
        ...
        const double at_least_light_precip = proportion_gteq_x(hourly_precip, 1);
        CHECK(at_least_light_precip >= .025);
        CHECK(at_least_light_precip <= .05);

        // Likewise for heavy precipitation.
        const double heavy_precip = proportion_gteq_x(hourly_precip, 2.5);
