...
public:
    vehicle_cursor(vehicle& veh, std::ptrdiff_t part, bool ignore_vpart = false)
        : veh(veh),
          part(part),
          ignore_vpart(ignore_vpart) {}
    vehicle& veh;
    std::ptrdiff_t part;
    bool ignore_vpart;
};
...
public:
     ...
     */
    vehicle_selector(
        const tripoint_bub_ms& pos, int radius = 0, bool accessible = true,
