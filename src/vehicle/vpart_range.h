 ...
 *   should be skipped).
 */
template <typename range_type> class vehicle_part_iterator {
private:
    ...
    auto skip_to_next_valid(size_t i) -> void {
        while (i < range_.part_count() && !range_.matches(i)) { ++i; }
...
public:
    ...
    auto operator==(const vehicle_part_iterator& rhs) const -> bool {
        if (vp_.has_value() != rhs.vp_.has_value()) { return false; }
        if (!vp_.has_value()) { return true; }
...
protected:
    ...
    generic_vehicle_part_range(::vehicle& v)
        : vehicle_(&v) {} // NOLINT(bugprone-crtp-constructor-accessibility)

public:
    // Templated because see top of file.
    template <typename T = ::vehicle> auto part_count() const -> size_t {
        return static_cast<const T&>(*vehicle_).part_count();
    }

    using iterator = vehicle_part_iterator<range_type>;

    auto begin() const -> iterator { return iterator(static_cast<const range_type&>(*this), 0); }
...
class vehicle_part_with_feature_range
    : public generic_vehicle_part_range<vehicle_part_with_feature_range<feature_type>> {
private:
...
public:
    ...
    auto empty() const -> bool { return this->begin() == this->end(); }
};
...
template <> inline constexpr bool enable_borrowed_range<vehicle_part_range> = true;
